...
public class Facturabase {
    ...
    public int IdFactura(){
        int id = 0;
        String sql = "SELECT MAX(id) FROM facturas";
        try {
    ...
    public int RegistrarFactura(Factura f){
        String sql = "INSERT INTO ventas (cliente, Comercial, total, fecha) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, f.getCliente());
            ps.setString(2, f.getComercial());
    ...
    public int RegistrarDetalle(tipotrasporte Dv){
       String sql = "INSERT INTO detalle (id_pro, cantidad, precio, id_Factura) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List ListarFactura(){
       ...
       String sql = "SELECT c.id AS id_cli, c.nombre, f.* FROM clientes c INNER JOIN ventas v ON c.id = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
           while (rs.next()) {               
               ...
               fac.setNombre_cli(rs.getString("nombre"));
               fac.setComercial(rs.getString("vendedor"));
    ...
    public Factura BuscarFactura(int id){
        ...
        try {
            ...
            if (rs.next()) {
                ...
                cl.setComercial(rs.getString("comercial"));
    ...
    public void pdfV(int idFactura, int Cliente, double total, String usuario) {
        try {
            Date date = new Date();
            FileOutputStream archivo;
            String url = FileSystemView.getFileSystemView().getDefaultDirectory().getPath();
            File salida = new File(url + "Factura.pdf");
